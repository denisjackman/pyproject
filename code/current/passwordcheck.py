...
def read_password_file():
    ...
    with open(r'Y:/Resources/project/password.txt', 'r', encoding='utf-8-sig') as passwordfile:
...
def password_integrity(password):
    ...
    if isupper < MINUPPER:
        result = False
        print(f"[FAIL] Password - {password} is missing uppercase characters {isupper}")
    if isupper < MINALPHA:
        result = False
        print(f"[FAIL] Password - {password} is missing lowerase characters {isupper}")
    if isupper < MINNUMBERS:
        result = False
        print(f"[FAIL] Password - {password} is missing numbers {isupper}")
    if isupper < MINSYMBOLS:
        result = False
        print(f"[FAIL] Password - {password} is missing symbols {isupper}")
    return result
