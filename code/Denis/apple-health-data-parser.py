...
class HealthDataExtractor():
    ...
    def count_record_types(self):
        ...
        for record in self.nodes:
            if record.tag == 'Record':
                ...
                self.report(f'Unexpected node of type {record.tag}.')
    ...
    def open_for_writing(self):
        ...
        for kind in (list(self.record_types) + list(self.other_types)):
            path = os.path.join(self.directory, f'{abbreviate(kind)}.csv')
            f = open(path, 'w', encoding='utf-8-sig')
            headerType = (kind if kind in ('Workout', 'ActivitySummary')
                          else 'Record')
            f.write(','.join(FIELDS[headerType].keys()) + '\n')
            self.handles[kind] = f
            self.report(f'Opening {path} for writing')
    ...
    def close_files(self):
        ...
        for (kind, f) in self.handles.items():
            f.close()
            self.report(f'Written {abbreviate(kind)} data.')
