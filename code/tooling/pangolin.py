...
def find_duplicate_files(directory_path):
    ...
    for root, directories, files in os.walk(directory_path):  # pylint: disable=unused-variable  # noqa: E501
        for item_file in tqdm(files, total=len(files), unit=' item_file'):
            file_count += 1
            # Calculate the hash of the file.
            file_path = os.path.join(root, item_file)
            try:
                file_hash = hashlib.md5(open(file_path, "rb").read()).hexdigest()  # pylint: disable=consider-using-with  # noqa: E501
            except OSError:
                print(f'[-] {file_path} already exists in {PANGOLIN_TARGET_DIR}')  # noqa: E501

            # If the hash is already in the
            # dictionary, the file is a duplicate.
            if file_hash in file_hashes:
                print(f'[0] Duplicate found: {file_path} {file_hashes[file_hash]}')  # noqa: E501
                count += 1
