...
def main():
    ...
    datadelete = True
    datalist = []
    datalist.append({   "_id" : "U1IT00012",   "item_name" : "even more new Stuff",   "category" : "stuff",   "quantity" : 12,   "price" : 40,   "item_description" : "a bunch of stuff" })
    if datainsert:
        insertData(client, 'user_shopping_list', 'user_1_items', datalist)

    for item in getData(client, 'user_shopping_list', 'user_1_items'):
        if item['_id'] in ('U1IT00012', 'U1IT00011'):
            print(item)

    info = updateData(client, 'user_shopping_list', 'user_1_items', 'U1IT00011', 'item_name', 'Updated stuff')
    print(f"[-] Update Result is {info.modified_count}")
    if datadelete:
        info = deleteData(client, 'user_shopping_list', 'user_1_items', 'U1IT00012')
