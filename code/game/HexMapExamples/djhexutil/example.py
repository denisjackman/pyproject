...
class Level():
    ...
    def get_tile(self, hexagon):
        '''Get the tile at the given hexagon.'''
    ...
    def is_passable(self, hexagon):
        '''Return True if the given hexagon is passable.'''
        return self.get_tile(hexagon) not in '#~'

    def is_transparent(self, hexagon):
        '''Return True if the given hexagon is transparent.'''
        return self.get_tile(hexagon) != '#'

    def update_fov(self, fov):
        '''Update the seen tiles based on the given field of view.'''
        for hexagon in fov:
...
class GameWidget(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, event):
        '''Select the hexagon under the mouse.'''
        self.select_hexagon(event.pos())
