...
# Each square is defined by two integer
# co-ordinates x and y. x is right, and y is up.
# Following standard conventions, the square with
# co-ordinate (0, 0) spans
...
def square_rotate_90(x, y, n=1):
    """
    Rotates the given square n * 90 degrees counter clockwise around the
    (0, 0) square, and returns the co-ordinates of the new square.
    """
    n = mod(n, 4)
    if n == 0:
        return (x, y)
...
def square_rotate_about_90(x, y, about_x, about_y, n=1):
    """
    Rotates the given square n * 90 degress counter clockwise
    about the given square and return the co-ordinates of the new square.
    """
...
def square_reflect_by(x, y, n=0):
    """
    Reflects the given square through the x-axis
    rotated counter clockwise by n * 45 degrees
    and returns the co-ordinates of the new square"""
...
def square_line(x1, y1, x2, y2):
    """
    Returns the squares in a shortest path from one square to another,
...
def square_rect_intersect(x, y, width, height):
    """
    Returns the square that intersect the rectangle specified in
...
def square_rect(rect_x, rect_y, width, height):
    """
    Returns the squares in a rectangle that includes the given
    sququre in the bottom left, that extends `height` squares
    upwards, and `width` squares to the right.
    """
...
def square_rect_knoll(x, y, rect_x, rect_y, width, height):
    """
    Given a square and a rectangle, gives a pair of integer
    cartesian co-ordinates that identify the square in the rectangle
    """
    dwidth = 0
    dwidth += width + height
...
def square_parent_rect(x, y):
    """
    Returns the left, bottom, width, height of th
    rect of a given parent square.
    """
    return (x * PARENT_WIDTH,
            y * PARENT_HEIGHT,
            PARENT_WIDTH,
            PARENT_HEIGHT)
