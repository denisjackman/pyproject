...
# for an extended explnation of how this works.
# This module provides sample code for working with regular
# hexagons flat-topped configuration, i.e.
...
# x + y + z always sums to 0, so z is redundant and can be
# omitted when storing co-ordinates.
# There are many other possible co-ordinate schemes, but this
# one seems to have the simplest maths.

# To find the neighbours of a given hex, add one to one co-ordinate,
...
def hex_center(x, y, z):
    ...
    # Each unit of x, y, z moves you in the direction of one of the corners
    # of the hex, in linear combination.
    #
    # NB: This function has the nice property that if you pass in x,y,z
    # values that sum to 1 or -1 (not a valid hex), it'll return co-ordinates
    # for the vertices of the hexes.
...
def tri_to_hex(x, y, z):
    ...
    finds the hex that contains it
    """
...
def hex_to_tris(x, y, z):
    """
    Given a hex, returns the co-ordinates of the 6 triangles it contains,
...
def hex_rotate_60(x, y, z, n=1):  # pylint: disable=R0911
    """
    Rotates the given hex n * 60 degrees counter clockwise around
    the origin,and returns the co-ordinates of the new hex.
    """
    n = mod(n, 6)
    if n == 0:
        return (x, y, z)
...
def hex_rotate_about_60(x, y, z, about_x, about_y, about_z, n=1):
    """
    Rotates the given hex n* 60 degress counter clockwise about the
    given hex and return the co-ordinates of the new hex.
    """
...
def hex_reflect_by(x, y, z, n=0):
    """
    Reflects the given hex through the x-axis rotated counter clockwise by
    n * 30 degrees and returns the co-ordinates of the new hex
    """
...
def hex_line(x1, y1, z1, x2, y2, z2):
    """
    Returns the hexes in a shortest path from one hex to another, staying
    as close to the straight line as possible
    """
    # Note that drawing a straight line from one hex to another can touch
    # hexes not returned by this method.
...
def hex_rect_intersect(x, y, width, height):
    """
    Returns the hexes that intersect the rectangle specified in cartesian
    co-ordinates
    """
    prev = None
    first_b = None
...
def hex_rect(rect_x,
             ...
             inc_top=False):
    """
    Returns the hexes in a rectangle that includes the given
    hex in the bottom left, that extends `height` hexes upwards,
    and `width` hexes to the right.inc_bottom and inc_top increase
    the size of the rect in every other column
    """
...
# Groups hexes into larger shapes that is each a disc around a given center
# hex These parent discs are themselves roughly hexagon shaped and roughly
# laid out like a pointy topped hexagon grid.
