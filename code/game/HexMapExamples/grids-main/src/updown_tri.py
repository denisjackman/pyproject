...
# UpDown Triangle Co-ordinates
# This module provides sample code for working with
# equilateral triangles in an up-down configuration, i.e.
#        ____________
#       /\    /\    /
#      /  \  /  \  /
#     /____\/____\/
...
# b determines which row the triangle is in, and a and c
# the two diagonals. a + b + c always sums to either 1 or 2.
...
# To find the neighbours of a down triangle, add 1 to a co-ordinate,
# and subtract one for neighbours of an up triangle.
...
def tri_center(a, b, c):
    ...
    # Or equivalently, this function multiplies by the inverse matrix to
    # pick_tri
    #
    # NB: This function has the nice property that if you pass in x,y,z
    # values that sum to zero (not a valid triangle), it'll return
    # co-ordinates for the vertices of the triangles.
...
def tri_corners(a, b, c):
    """Returns the three corners of a
    given triangle in cartesian co-ordinates"""
    if points_up(a, b, c):
        return [
            tri_center(1 + a, b, c),
            tri_center(a, b, 1 + c),
...
def pick_tri(x, y):
    """Returns the triangle that contains a
    given cartesian co-ordinate point"""
    # Using dot product, measures which row and
    # diagonals a given point occupies.
    # Or equivalently, multiply by the inverse matrix to tri_center
    # Note we have to break symmetry, using floor(...)+1
    # instead of ceil, in order
...
def tri_rotate_60(a, b, c, n=1):  # pylint: disable=R0911
    """Rotates the given triangle n * 60 degrees counter clockwise around
    the origin, and returns the co-ordinates of the new triangle."""
    n = mod(n, 6)
    if n == 0:
        return (a, b, c)
    if n == 1:
...
def tri_rotate_about_60(a, b, c, about_a, about_b, about_c, n=1):
    """Rotates the given triangle n* 60 degress counter clockwise
    about the given tri and return the co-ordinates of the new triangle."""
...
def tri_reflect_by(a, b, c, n=0):
    """Reflects the given triangle through the x-axis rotated counter
    clockwise by n * 30 degrees and returns the
    co-ordinates of the new triangle"""
...
def tri_line(a1, b1, c1, a2, b2, c2):
    """
    Returns the tris in a shortest path from one tri to another
    , staying as close to the straight line as possible"""
...
def tri_rect_intersect(x, y, width, height):
    """Returns the tris that intersect the rectangle specified
    in cartesian co-ordinates"""
    assert width >= 0, "Rectangle should have non-negative width"
    assert height >= 0, "Rectangle should have non-negative height"
    # For consistency, we treat the triangles as exclusive of their
    # border, and the rect as inclusive
    x /= edge_length
    y /= edge_length
    width /= edge_length
    height /= edge_length
