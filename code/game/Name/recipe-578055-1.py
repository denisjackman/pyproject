...
def adduser():
    ...
    while True:
        ...
        if k=='':
            break
        a.append(k)
        with open('random_members.data', 'wb') as f:
            pickle.dump(a, f)
...
def deluser():
    ...
    while True:
        ...
        for l in range(0,len(a)):
            if k==a[l-1]:
                del a[l-1]
                with open('random_members.data', 'wb') as fdump:
                    pickle.dump(a, fdump)
if os.path.isfile('random_members.data') is not True:
    ...
    with open('random_members.data', 'rb') as fload:
        a = pickle.load(fload)
