...
def hex_add(a, b):
    ''' hex_add(a, b) returns a new Hex object that is the sum of Hex objects a and b.'''
    return Hex(a.q + b.q, a.r + b.r, a.s + b.s)

def hex_subtract(a, b):
    ''' hex_subtract(a, b) returns a new Hex object that is the difference of Hex objects a and b.'''
    return Hex(a.q - b.q, a.r - b.r, a.s - b.s)

def hex_scale(a, k):
    ''' hex_scale(a, k) returns a new Hex object that is the product of Hex object a and scalar k.'''
    return Hex(a.q * k, a.r * k, a.s * k)

def hex_rotate_left(a):
    ''' hex_rotate_left(a) returns a new Hex object that is the result of rotating Hex object a 60 degrees counter-clockwise.'''
    return Hex(-a.s, -a.q, -a.r)

def hex_rotate_right(a):
    ''' hex_rotate_right(a) returns a new Hex object that is the result of rotating Hex object a 60 degrees clockwise.'''
...
def hex_round(h):
    ''' hex_round(h) returns a new Hex object that is the rounded version of Hex object h.'''
    qi = int(round(h.q))
    ri = int(round(h.r))
    si = int(round(h.s))
    q_diff = abs(qi - h.q)
...
def hex_lerp(a, b, t):
    ''' hex_lerp(a, b, t) returns a new Hex object that is the linear interpolation between Hex objects a and b at time t.'''
    return Hex(a.q * (1.0 - t) + b.q * t, a.r * (1.0 - t) + b.r * t, a.s * (1.0 - t) + b.s * t)

def hex_linedraw(a, b):
    ''' hex_linedraw(a, b) returns a list of Hex objects that are on the line between Hex objects a and b.'''
    N = hex_distance(a, b)
...
def qoffset_to_cube(offset, h):
    ''' qoffset_to_cube(offset, h) returns a new Hex object that is the conversion of OffsetCoord object h to a cube coordinate.'''
    q = h.col
...
def roffset_to_cube(offset, h):
    ''' roffset_to_cube(offset, h) returns a new Hex object that is the conversion of OffsetCoord object h to a cube coordinate.'''
...
def qdoubled_from_cube(h):
    ''' qdoubled_from_cube(h) returns a DoubledCoord object that is the conversion of Hex object h to a doubled coordinate.'''
    col = h.q
    row = 2 * h.r + h.q
    return DoubledCoord(col, row)

def qdoubled_to_cube(h):
    ''' qdoubled_to_cube(h) returns a new Hex object that is the conversion of DoubledCoord object h to a cube coordinate.'''
    q = h.col
    r = (h.row - h.col) // 2
    s = -q - r
    return Hex(q, r, s)
...
def rdoubled_from_cube(h):
    ''' rdoubled_from_cube(h) returns a DoubledCoord object that is the conversion of Hex object h to a doubled coordinate.'''
    col = 2 * h.q + h.r
    row = h.r
    return DoubledCoord(col, row)

def rdoubled_to_cube(h):
    ''' rdoubled_to_cube(h) returns a new Hex object that is the conversion of DoubledCoord object h to a cube coordinate.'''
    q = (h.col - h.row) // 2
    r = h.row
    s = -q - r
    return Hex(q, r, s)
...
def hex_to_pixel(layout, h):
    ''' hex_to_pixel(layout, h) returns a Point object that is the conversion of Hex object h to a pixel coordinate.'''
...
def pixel_to_hex(layout, p):
    ''' pixel_to_hex(layout, p) returns a new Hex object that is the conversion of Point object p to a hex coordinate.'''
    M = layout.orientation
    size = layout.size
    origin = layout.origin
...
def hex_corner_offset(layout, corner):
    ''' hex_corner_offset(layout, corner) returns a Point object that is the conversion of a corner number to a pixel offset.'''
    M = layout.orientation
    size = layout.size
...
def polygon_corners(layout, h):
    ''' polygon_corners(layout, h) returns a list of Point objects that are the corners of the hexagon h.'''
    corners = []
...
def equal_offsetcoord(name, a, b):
    ''' equal_offsetcoord(name, a, b) checks that a and b are equal OffsetCoord objects.'''
    if not (a.col == b.col and a.row == b.row):
        complain(name)

def equal_doubledcoord(name, a, b):
    ''' equal_doubledcoord(name, a, b) checks that a and b are equal DoubledCoord objects.'''
    if not (a.col == b.col and a.row == b.row):
        complain(name)
...
def equal_hex_array(name, a, b):
    ''' equal_hex_array(name, a, b) checks that a and b are equal lists of Hex objects.'''
...
def test_hex_neighbor():
    ''' hex_neighbor(a, direction) returns the hex that is adjacent to hex a in the given direction'''
    equal_hex("hex_neighbor", Hex(1, -3, 2), hex_neighbor(Hex(1, -2, 1), 2))

def test_hex_diagonal():
    ''' hex_diagonal_neighbor(a, direction) returns the hex that is diagonally across from hex a in the given direction'''
...
def test_hex_rotate_right():
    ''' hex_rotate_right(a) returns the hex that is a rotated right around the origin hex'''
