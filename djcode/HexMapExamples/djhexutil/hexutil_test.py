...
class TestHexGrid(unittest.TestCase):
        ...
        for fx in (-1, 1):
            for fy in (-1, 1):
                for pixel, pxhex in data:
                    ...
                    self.assertEqual(hg.hex_at_coordinate(*pixel), pxhex, pixel)
    ...
    def test_bounding_box(self):
        ''' Test that hexutil.HexGrid.bounding_box returns the correct bounding boxes.'''
        hg = hexutil.HexGrid(32)
        self.assertEqual(hg.bounding_box(hexutil.Hex(0,2)),
                hexutil.Rectangle(-32, 72, 64, 72))

    def test_hexes_in_rectangle(self):
        ''' Test that hexutil.HexGrid.hexes_in_rectangle returns the correct hexes.'''
        hg = hexutil.HexGrid(32)
        self.assertEqual(
